    ...
    try:
      ...
      if version in _SERVICE_MAP:
        raise googleads.errors.GoogleAdsValueError(
            ...
            % (service_name, list(_SERVICE_MAP[version].keys())))
      else:
        raise googleads.errors.GoogleAdsValueError(
            'Unrecognized version of the AdWords API. Version given: %s '
            'Supported versions: %s' % (version, list(_SERVICE_MAP.keys())))
...
class ReportDownloader(object):
  ...
  def __init__(self, adwords_client, version=sorted(_SERVICE_MAP.keys())[-1],
    ...
    # Create an Opener to handle requests when downloading reports.
      self.url_opener = urllib.request.build_opener(
          urllib.request.ProxyHandler({'https': self._adwords_client.https_proxy}))
    else:
      self.url_opener = urllib.request.build_opener()
  ...
  def _SerializeAwql(self, query, file_format):
    ...
    """
    return urllib.parse.urlencode({'__fmt': file_format, '__rdquery': query})
  ...
  def _SerializeReportDefinition(self, report_definition):
    ...
    return urllib.parse.urlencode({'__rdxml': self._marshaller.process(content)})
