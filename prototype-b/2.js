...
let player = {
    ...
    isVisible: true // To hide player during explosion
};
...
// Explosion tracking
let activeExplosions = []; // Stores { x, y, timer, duration }
...
function draw() {
    ...
    // If game is not paused AND not currently exploding
    if (!gameState.isPaused && activeExplosions.length === 0) { 
        updatePlatforms(); 
        updatePlayer();    
        updateClouds(); 
    } else if (activeExplosions.length > 0) {
        // Only update explosions if an explosion is active
        updateExplosions();
        // Keep updating clouds for visual consistency during explosion
        updateClouds(); 
    } else {
        // Game is paused, show start prompt
        drawStartPrompt();
    }
...
function drawObstacles() {
    // Draw bombs instead of red squares
    for (let obstacle of obstacles) {
        let screenX = obstacle.x - (player.x - SCREEN_WIDTH / 2);
        
        // Bomb body (black circle)
        fill(10, 10, 10);
        ellipse(screenX, obstacle.y + obstacle.height * 0.1, obstacle.width * 0.8, obstacle.height * 0.8);
        
        // Fuse (simple gray line with a spark)
        stroke(100);
        strokeWeight(3);
        line(screenX, obstacle.y + obstacle.height * 0.1 - obstacle.height * 0.4, 
             screenX + obstacle.width * 0.2, obstacle.y + obstacle.height * 0.1 - obstacle.height * 0.6);
        noStroke();
        fill(255, 150, 0); // Spark color
        ellipse(screenX + obstacle.width * 0.2, obstacle.y + obstacle.height * 0.1 - obstacle.height * 0.6, 5, 5);
    }
...
function updatePlayer() {
    ...
    // 7. Update and Check Obstacles (Bombs)
    for (let i = obstacles.length - 1; i >= 0; i--) {
        let obstacle = obstacles[i];
        obstacle.x -= worldScrollAmount; // Apply world scroll

        // Use obstacle width/height for collision box approximation
        if (player.isVisible && // Only collide if player is visible
            player.x + NOTE_SIZE/2 > obstacle.x - obstacle.width/2 && // Check against center of bomb approx
            player.x - NOTE_SIZE/2 < obstacle.x + obstacle.width/2 &&
            player.y + NOTE_SIZE/2 > obstacle.y - obstacle.height/2 &&
            player.y - NOTE_SIZE/2 < obstacle.y + obstacle.height/2) 
        {
            // --- Player hit a bomb! --- 
            console.log("Boom!");
            player.isVisible = false; // Hide player
            player.velocityY = 0; // Stop player movement
            player.speed = 0; // Stop world scrolling
            
            // Create explosion effect at bomb location
            activeExplosions.push({
                x: obstacle.x, // Use obstacle's world x
                y: obstacle.y, // Use obstacle's world y
                timer: EXPLOSION_DURATION,
                duration: EXPLOSION_DURATION
            });
            
            // Remove the bomb obstacle
            obstacles.splice(i, 1);
            
            // Don't respawn yet, just return to let explosion play
            return; 
        }
...
function respawnPlayer() {
    ...
    // Game is paused by loadLevel, player needs to press space to unpause
}

function drawPlayer() {
    if (!player.isVisible) return; // Don't draw if invisible (during explosion)
    ...
    rectMode(CENTER); // Reset rectMode if needed elsewhere
}
...
function loadLevel(levelNum) {
    ...
    player.isVisible = true; // Make player visible again
    gameState.isAccelerating = false;
    gameState.isDecelerating = false;
    gameState.isTutorial = levelNum === 1; 
    gameState.isPaused = true;  
    gameState.isGameOver = false; 
    activeExplosions = []; // Clear any active explosions
    setupClouds(); 
}
...
function updateExplosions() {
    for (let i = activeExplosions.length - 1; i >= 0; i--) {
        ...
        // Update position based on world scroll (so explosion stays in place relative to world)
        // Note: player.speed should be 0 when explosion starts, so scroll is 0
        // If we allow scrolling during explosion, this needs adjustment.
        // exp.x -= player.speed; 

        if (exp.timer <= 0) {
            // Explosion finished
            activeExplosions.splice(i, 1);
            // If this was the last active explosion, trigger respawn
...
function drawExplosions() {
    for (let exp of activeExplosions) {
        let progress = (exp.duration - exp.timer) / exp.duration; // 0 to 1
        let alpha = map(progress, 0, 1, 255, 0); // Fade out
        let radius = map(progress, 0, 1, 5, 80); // Expand
        
        // Calculate screen position
        let screenX = exp.x - (player.x - SCREEN_WIDTH / 2);
        let screenY = exp.y;

        // Draw expanding circles (simple effect)
        noStroke();
        // Orange center
        fill(255, 150, 0, alpha);
        ellipse(screenX, screenY, radius * 0.8, radius * 0.8);
        // Yellow outer
        fill(255, 255, 0, alpha * 0.7);
        ellipse(screenX, screenY, radius, radius);
        // Red sparks (random)
        fill(255, 0, 0, alpha);
        for (let j = 0; j < 5; j++) {
             let angle = random(TWO_PI);
             let sparkDist = random(radius * 0.5, radius * 1.2);
             ellipse(screenX + cos(angle) * sparkDist, screenY + sin(angle) * sparkDist, 5, 5);
         }
    }
}